...
export default function AdminCMSModal({
  ...
  const [previewDir, setPreviewDir] = useState('rtl');
  const [editorViewMode, setEditorViewMode] = useState('preview'); // 'preview' | 'code'
  ...
  const handleDocxUpload = async (e) => {
    ...
    try {
      ...
      setPreviewDir(isRTL ? 'rtl' : 'ltr');
      setEditorViewMode('preview');
      setDocxSuccessMsg(`Successfully extracted template from "${file.name}"! You can review and edit it below before publishing.`);
    } catch (err) {
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-2xs space-y-4">
                    ...
                    <form onSubmit={handleCreateTemplate} className="space-y-4 text-xs">
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        ...
                        <div>
                          ...
                          >
                            ...
                            <option value="Lisan al Dawat">Lisan al Dawat</option>
                      ...
                      {/* Template Content & Live A4 Preview */}
                      <div className="space-y-2">
                        <div className="flex items-center justify-between border-b border-gray-200 pb-2 flex-wrap gap-2">
                          <label className="text-gray-800 font-bold flex items-center gap-1.5">
                            ...
                            <span>Live Template Preview & Editor</span>
                          </label>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              type="button"
                              onClick={() => {
                                const nextDir = previewDir === 'rtl' ? 'ltr' : 'rtl';
                                setPreviewDir(nextDir);
                                if (nextDir === 'rtl') {
                                  setNewTplContent((prev) => prev.replace(/<table(?![^>]*dir=)/gi, '<table dir="rtl"'));
                                }
                              }}
                              className={`px-2.5 py-1 rounded text-xs font-bold border transition-colors ${
                                previewDir === 'rtl' ? 'bg-emerald-50 border-emerald-300 text-emerald-800' : 'bg-gray-100 border-gray-300 text-gray-700'
                              }`}
                              title="Toggle RTL / LTR layout"
                            >
                              ⇄ {previewDir === 'rtl' ? 'RTL (Right to Left)' : 'LTR (Left to Right)'}
                            </button>
                            ...
                            <div className="flex items-center bg-gray-100 p-0.5 rounded border border-gray-300">
                              <button
                                ...
                                className={`px-2.5 py-0.5 rounded text-xs font-semibold flex items-center gap-1 transition-colors ${
                                  editorViewMode === 'preview' ? 'bg-white text-blue-900 shadow-2xs' : 'text-gray-600 hover:text-gray-900'
                                }`}
                              >
                                <Eye className="w-3.5 h-3.5 text-blue-600" />
                                <span>A4 Preview (Editable)</span>
                              </button>
                              <button
                                type="button"
                                onClick={() => setEditorViewMode('code')}
                                className={`px-2.5 py-0.5 rounded text-xs font-semibold flex items-center gap-1 transition-colors ${
                                  editorViewMode === 'code' ? 'bg-white text-blue-900 shadow-2xs' : 'text-gray-600 hover:text-gray-900'
                                }`}
                              >
                        ...
                        {/* Interactive A4 Document Preview Card */}
                        {editorViewMode === 'preview' ? (
                          <div className="bg-slate-100 p-4 rounded-xl border border-gray-300 flex flex-col items-center">
                            <div className="text-[11px] text-gray-500 mb-2 flex items-center gap-1.5 self-start">
                              <Edit3 className="w-3.5 h-3.5 text-blue-500" />
                              <span>Click anywhere directly inside this A4 sheet to edit text and table contents in real-time before publishing:</span>
                            </div>
                            ...
                            <div 
                              ...
                              onBlur={(e) => {
                                setNewTplContent(e.currentTarget.innerHTML);
                              }}
                              dangerouslySetInnerHTML={{ __html: newTplContent || '<p class="text-gray-400 italic">No content yet. Type here or upload a Word (.docx) file above...</p>' }}
                            />
                          </div>
                        ) : (
                          ...
                          <div>
                            <textarea
                              ...
                              required
                              className="w-full px-3 py-2 border border-gray-300 rounded font-mono text-xs focus:ring-1 focus:ring-slate-900 bg-slate-900 text-emerald-400"
                            />
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex items-center justify-between pt-2 border-t border-gray-100">
                        <button
                          ...
                          className="text-gray-500 hover:text-red-600 font-semibold text-xs px-2 py-1"
                        >
                          Clear Form
                        </button>
                        ...
                        <button
                          type="submit"
                          className="bg-emerald-700 hover:bg-emerald-800 text-white px-5 py-2 rounded-lg font-bold text-xs shadow-md transition-colors flex items-center gap-2"
                        >
                          <Check className="w-4 h-4 text-emerald-200" />
                          <span>Publish Template to Cloud & Catalog</span>
                        </button>
                      </div>
                    </form>
                  </div>
